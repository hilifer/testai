...
pub enum ProviderKind {
    ...
    Xai,
    OpenAi,
    /// Web provider — uses browser credentials via the built-in web-bridge gateway.
    Web,
}
    ...
    MODEL_REGISTRY
        ...
        .find_map(|(alias, metadata)| {
            (*alias == lower).then_some(match metadata.provider {
                ...
                ProviderKind::OpenAi | ProviderKind::Web => trimmed,
            })
        })
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    // Web provider models: web/*, deepseek/*, chatgpt/*, gemini/*, kimi/*
    if web_bridge::is_web_model(&canonical) {
        return Some(ProviderMetadata {
            provider: ProviderKind::Web,
            auth_env: "WEB_BRIDGE_TOKEN",
