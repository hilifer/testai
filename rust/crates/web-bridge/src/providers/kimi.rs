...
const BASE_URL: &str = "https://kimi.moonshot.cn";
const CHAT_API: &str = "https://kimi.moonshot.cn/api/chat/completions";
...
impl WebProvider for KimiProvider {
    fn name(&self) -> &str {
        "kimi"
    }
    ...
    > {
        ...
        let stream = KimiSseStream::new(response.bytes_stream());
        Ok(Box::new(stream))
    }
}

struct KimiSseStream {
...
impl tokio_stream::Stream for KimiSseStream {
            ...
            if let Some(newline_pos) = self.buffer.find('\n') {
                ...
                if let Some(data) = line.strip_prefix("data: ") {
                    ...
                    if let Ok(parsed) = serde_json::from_str::<Value>(data) {
                        // Kimi uses standard OpenAI-like delta format
                        let choice = parsed
                            .get("choices")
