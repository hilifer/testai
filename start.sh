...
# Read gateway token from config (auto-generated by openclaw)
read_gateway_token() {
    python3 -c "
import json
try:
...
gateway_alive() {
    if [ -n "$GATEWAY_TOKEN" ]; then
        curl -sf --connect-timeout 2 -H "Authorization: Bearer ${GATEWAY_TOKEN}" "http://localhost:${GATEWAY_PORT}/v1/models" >/dev/null 2>&1
    else
        curl -sf --connect-timeout 2 "http://localhost:${GATEWAY_PORT}/v1/models" >/dev/null 2>&1
    fi
}
...
if gateway_alive; then
    ...
    info "Starting openclaw-zero-token gateway..."
    cd "${OPENCLAW_DIR}"
    bash ./server.sh start 2>&1 | tail -5 || true
    cd "${HERE}"

    # Re-read token (may have been generated during startup)
    GATEWAY_TOKEN=$(read_gateway_token)

    # Wait up to 90 seconds
    GATEWAY_OK=false
    info "Waiting for gateway (may take up to 60s on first start)..."
    ...
    if $GATEWAY_OK; then
        ...
        warn "Gateway health check failed, may still be starting"
        warn "Log: tail -f /tmp/openclaw/openclaw-$(date +%Y-%m-%d).log"
    fi
fi

# Final token read
GATEWAY_TOKEN=$(read_gateway_token)
ok "Gateway token: ${GATEWAY_TOKEN:0:8}..."

# ============================================================
# Step 6: Start claw CLI connected to openclaw gateway
# ============================================================
info "Step 6/6: Starting claw CLI..."
...
# Connect claw to openclaw gateway via OpenAI-compatible API
